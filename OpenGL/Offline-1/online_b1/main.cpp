...
double surface_inclination_angle = 40;
...
void draw_direction_vector_forward()
{
    glColor3f(1,0,0);
    glBegin(GL_LINES);
    {
...
void draw_direction_vector_side()
{
    glColor3f(0,1,0);
    glBegin(GL_LINES);
    {
...
void drawSS()
{
    ...
    draw_direction_vector_forward();
    glPopMatrix();

    glPushMatrix();
    glRotatef(leftRightRotate_angle, 0, 0, 1);
    draw_direction_vector_side();
    glPopMatrix();
...
void keyboardListener(unsigned char key, int x,int y){
	switch(key){
        case 'w':
            ...
            if(forward_backward.x <= -30)
            {
                forward_backward.z += one_click_dist*(tan(surface_inclination_angle*pi/180));
            }
            else forward_backward.z = 0;
			break;
        case 's':
            rotate_angle+=rotate_angleStep;
            forward_backward.x += one_click_dist *(cos(leftRightRotate_angle*pi/180));
...
void display(){
	...
	****************************/
	...
	glTranslatef(-30, 0, 0);
	glRotatef(surface_inclination_angle, 0, 1, 0);
